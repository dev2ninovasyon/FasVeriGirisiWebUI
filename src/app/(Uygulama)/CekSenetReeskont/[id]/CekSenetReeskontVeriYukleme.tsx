...
const CekSenetReeskontVeriYukleme: React.FC<Props> = ({
  ...
  const handleBeforeChange = (changes: any[]) => {
    ...
    for (let i = 0; i < changes.length; i++) {
      ...
      // Sayısal Alanlar: 4 (Çek / Senet No), 6 (Nominal Değer)
      if ([4, 6].includes(prop)) {
      ...
      // Tarih Alanları: 2 (Kayıt Tarihi), 5 (Vade Tarihi)
      if ([2, 5].includes(prop)) {
        if (typeof newValue === "string" && newValue !== "") {
  ...
  const handleCreateCekSenetReeskontVerisi = async () => {
    const hasDataCallback = (item: any) => {
      // item[0]: Detay Hesap Kodu, item[1]: Hesap Adı, item[2]: Kayıt Tarihi, item[4]: Çek/Senet No
      return (
